...
async function resetShopData() {
  ...
  await prisma.$transaction([
    ...
    prisma.normalizationJob.deleteMany({ where: { shopDomain: SHOP_DOMAIN } }),
    prisma.ruleImportJob.deleteMany({ where: { shopDomain: SHOP_DOMAIN } }),
    prisma.packRule.deleteMany({ where: { shopDomain: SHOP_DOMAIN } }),
    prisma.syncedCompanyLocation.deleteMany({
      where: { shopDomain: SHOP_DOMAIN },
    }),
    prisma.syncedCompany.deleteMany({ where: { shopDomain: SHOP_DOMAIN } }),
    prisma.syncedVariant.deleteMany({ where: { shopDomain: SHOP_DOMAIN } }),
  ]);
}
