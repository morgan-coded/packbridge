...
This checklist uses the current official Shopify docs above.
...
- ✅ Demo seed data exists for a redacted Shopify development store via `npm run seed:demo`.
...
- ✅ Demo and onboarding videos are stored outside the public repository and linked only during App Store submission.
...
- ⚠️ Production host URL and Shopify Partner redirect URLs must be set to the final deployed app URL before submission.
- ⚠️ If using a vanity domain, test OAuth install/reinstall end to end before switching the Partner app URL.
